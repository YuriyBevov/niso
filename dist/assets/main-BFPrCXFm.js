...
if (sliders.length) {
  sliders.forEach((slider2) => {
    const topSlider = slider2.classList.contains("top-side-slider");
...
document.addEventListener("DOMContentLoaded", () => {
  const items2 = document.querySelectorAll(".sidemenu li");
  if (items2.length) {
    items2.forEach((item) => {
      item.addEventListener("click", (evt) => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (tables.length) {
    let length = tables.length, i, wrapper;
    for (i = 0; i < length; i++) {
